...
- **页面菜单 page menu**: PAGES 的 `⋯` 改用非阻塞弹出菜单 `openPopmenu()`（Rename/Set as Home/Delete），不再用 `prompt/confirm`。Rename 为列表内联输入（`renamingPageId`）；Delete 直接删（有 Undo，提示 ⌘/Ctrl+Z）。**今后避免用 alert/prompt/confirm 打断用户**，优先弹出菜单/内联编辑。
